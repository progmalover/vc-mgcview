...
void CBCGPReportRow::OnDrawPreview (CDC* pDC, CRect rect)
{
	...
	int dx = m_pWndList->IsSortingMode () && !m_pWndList->IsGrouping () ? 0 : GetHierarchyLevel () * m_pWndList->m_nRowHeight;
...
void CBCGPReportRow::OnPrintName (CDC* pDC, CRect rect)
{
	...
	if (IsGroup ())
	{
		rectText.DeflateRect (0, (m_pWndList->m_nLargeRowHeight - m_pWndList->m_nRowHeight) / 2 , 0, 0);
	}
...
#else
	m_ColorData.m_GroupColors.m_clrBackground = afxGlobalData.clrBtnLight;
#endif
