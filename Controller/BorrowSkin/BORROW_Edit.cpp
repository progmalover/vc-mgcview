...
void CBCGPEdit::OnDrawBrowseButton (CDC* pDC, CRect rect, BOOL bIsButtonPressed, BOOL bHighlight)
{
	...
	if (m_bSearchMode && m_ImageSearch.IsValid() && m_bTextIsEmpty)
	{
		...
		m_ImageSearch.DrawEx(pDC, rect, 0, CMFCToolBarImages::ImageAlignHorzCenter, CMFCToolBarImages::ImageAlignVertCenter);
		return;
	}
	...
	if (!visualManager->OnDrawBrowseButton (pDC, rect, this, state, clrText))
	{
		return;
	}
	...
	if (m_bSearchMode && m_ImageClear.IsValid() && !m_bTextIsEmpty)
	{
		...
		if (bIsButtonPressed && visualManager->IsOffsetPressedButton ())
		{
			rectImage.left++;
			rectImage.top++;
		}

		m_ImageClear.DrawEx(pDC, rectImage, 0, CMFCToolBarImages::ImageAlignHorzCenter, CMFCToolBarImages::ImageAlignVertCenter);
		return;
	...
	if (m_ImageBrowse.IsValid())
	{
		...
		if (IsWindowEnabled())
		{
			m_ImageBrowse.DrawEx (pDC, rectImage, iImage, CMFCToolBarImages::ImageAlignHorzCenter, CMFCToolBarImages::ImageAlignVertCenter);
		}
		else
		{
			CAfxDrawState ds;
	...
	else if (!m_strLabel.IsEmpty ())
	{
		...
		if (m_bOnGlass)
		{
			visualManager->DrawTextOnGlass(pDC, m_strLabel, rectText, dwDTFlags, 0, clrText);
		}
		else
		{
...
void CBCGPEdit::EnableFileBrowseButton (LPCTSTR lpszDefExt/* = NULL*/, LPCTSTR lpszFilter/* = NULL*/)
{
	...
	EnableEditCtrlAutoComplete (GetSafeHwnd (), FALSE);
...
*/
	...
	EnableEditCtrlAutoComplete (GetSafeHwnd (), TRUE);
...
HBRUSH CBCGPEdit::CtlColor(CDC* pDC, UINT /*nCtlColor*/) 
{
	...
	{
		CBrush& br = visualManager->GetDlgBackBrush (GetParent());
...
void CBCGPEdit::EnableSearchMode(BOOL bEnable, LPCTSTR lpszPrompt)
{
	...
	if (m_bSearchMode)
	{
		...
		{
			CBORROW_LocalResource locaRes;
			m_ImageClear.Load(afxGlobalData.Is32BitIcons () ?
				IDB_AFXBARRES_CLEAR32 : IDB_AFXBARRES_CLEAR);
			m_ImageClear.SetSingleImage();
			m_ImageClear.SetTransparentColor(afxGlobalData.clrBtnFace);
		}
	}
	else
	{
