...
CString myGetMonthName(int month)
{
	CString szMonthName;
	...
	{
	case 1:
		szMonthName = Translate(_T("January"));
		break;
	case 2:
		szMonthName = Translate(_T("February"));
		break;
	case 3:
		szMonthName = Translate(_T("March"));
		break;
	case 4:
		szMonthName = Translate(_T("April"));
		break;
	case 5:
		szMonthName = Translate(_T("May"));
		break;
	case 6:
		szMonthName = Translate(_T("June"));
		break;
	case 7:
		szMonthName = Translate(_T("July"));
		break;
	case 8:
		szMonthName = Translate(_T("August"));
		break;
	case 9:
		szMonthName = Translate(_T("September"));
		break;
	case 10:
		szMonthName = Translate(_T("October"));
		break;
	case 11:
		szMonthName = Translate(_T("November"));
		break;
	case 12:
		szMonthName = Translate(_T("December"));
		break;
	}
	return szMonthName;
}

const UINT		CBORROW_DateTimeCtrl::DTM_SPIN			= 0x1;
const UINT		CBORROW_DateTimeCtrl::DTM_DATE			= 0x2;
const UINT		CBORROW_DateTimeCtrl::DTM_TIME			= 0x4;
const UINT		CBORROW_DateTimeCtrl::DTM_CHECKBOX		= 0x8;
const UINT		CBORROW_DateTimeCtrl::DTM_DROPCALENDAR = 0x10;
const UINT		CBORROW_DateTimeCtrl::DTM_TIME24H		= 0x20;
const UINT		CBORROW_DateTimeCtrl::DTM_CHECKED		= 0x40;
const UINT		CBORROW_DateTimeCtrl::DTM_TIME24HBYLOCALE = 0x80;
...
IMPLEMENT_DYNAMIC(CBORROW_DateTimeCtrl, CButton)

CBORROW_DateTimeCtrl::CBORROW_DateTimeCtrl()
{
	m_checkButton = FALSE;
	m_dropCalendar = TRUE;
	m_showDate = TRUE;
...
BEGIN_MESSAGE_MAP(CBORROW_DateTimeCtrl, CButton)
...
void CBORROW_DateTimeCtrl::DrawItem(LPDRAWITEMSTRUCT lpDIS) 
{
	ASSERT (lpDIS != NULL);
	...
	//rectClient.bottom = rectClient.top + m_arPartRects [0].Height();
	...
	else
	{
		pDC->FillSolidRect (&rectClient, 
			IsWindowEnabled () ? afxGlobalData.clrWindow : afxGlobalData.clrBtnFace);
	}
	...
	for (int i = iStart; i < m_iPartsNumber; i ++)
	{
		...
		/*if (m_arPartsOrder [i]  == MONTH)
		{
			str = Translate(myGetMonthName(m_Date.GetMonth()));			
		}*/
		...
		else
		{
			pDC->SetTextColor (m_bIsChecked && IsWindowEnabled () ? 
					(m_colorText == (COLORREF)-1 ? afxGlobalData.clrWindowText : m_colorText) : 
					afxGlobalData.clrGrayedText);
		}
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::OnShowCalendarPopup () 
{
	if (!m_dropCalendar)
		return;
...
//************************************************************************************
void CBORROW_DateTimeCtrl::OnLButtonDown(UINT /*nFlags*/, CPoint point) 
{
	m_iPrevDigit = -1;
...
//****************************************************************************************
void CBORROW_DateTimeCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	CButton::OnMouseMove(nFlags, point);
}
//****************************************************************************************
int CBORROW_DateTimeCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
...
//****************************************************************************************
UINT CBORROW_DateTimeCtrl::OnGetDlgCode() 
{
	return DLGC_WANTARROWS | DLGC_WANTCHARS;
}
//****************************************************************************************
void CBORROW_DateTimeCtrl::OnRButtonDown(UINT /*nFlags*/, CPoint /*point*/) 
{
}
//****************************************************************************************
void CBORROW_DateTimeCtrl::OnRButtonUp(UINT /*nFlags*/, CPoint /*point*/) 
{
}
//****************************************************************************************
void CBORROW_DateTimeCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
	if (!IsWindowEnabled ())
	{
		return;
...
//****************************************************************************************
DATE CBORROW_DateTimeCtrl::GetDate() 
{
	if (m_bIsChecked)
	{
...
//****************************************************************************************
void CBORROW_DateTimeCtrl::SetDate (DATE newValue) 
{
...
//****************************************************************************************
void CBORROW_DateTimeCtrl::SetMinDate(DATE newValue) 
{
...
//****************************************************************************************
DATE CBORROW_DateTimeCtrl::GetMaxDate() 
{
	return (DATE) m_MaxDate;
}
//****************************************************************************************
DATE CBORROW_DateTimeCtrl::GetMinDate() 
{
	return (DATE) m_MinDate;
}
//****************************************************************************************
void CBORROW_DateTimeCtrl::SetMaxDate(DATE newValue) 
{
...
//************************************************************************************************
void CBORROW_DateTimeCtrl::AdjustControl (CRect rectClient)
{
	if (GetSafeHwnd () == NULL)
	{
		return;
	...
	if (m_dropCalendar && m_showDate)
	{
		...
		//int iDropStart = rectClient.right - iDropButtonWidth -1;
		if (m_spinButton)
		{
		...
		m_rectDropButton = CRect (	iDropStart,
									...
									m_rectText.bottom);
		m_rectDropButton.DeflateRect(1,1);
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::DrawCheckBox (CDC* pDC, UINT uiState)
{
	if (!m_bCheckBoxIsAvailable)
	{
		return;
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::DrawDateDropButton (CDC* pDC, UINT uiState)
{
	if (!m_dropCalendar || !m_showDate)
	{
		return;
...
#ifndef _BCGSUITE_
	visualManager->OnDrawDateTimeDropButton (pDC, m_rectDropButton,
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::SelectNext ()
{
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::SelectPrev ()
{
	if (m_iPartNum == -1 || !m_bIsChecked)
	{
		return;
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::SetPartsOrder ()
{
	int i = 0;
...
//*****************************************************************************************
LPCTSTR CBORROW_DateTimeCtrl::GetPartFormat (int iPart) const
{
...
//*****************************************************************************************
int CBORROW_DateTimeCtrl::GetPartFromPoint (POINT point)
{
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::CalcPartRects (CDC* pDC)
{
	ASSERT_VALID (pDC);
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::ScrollCurrPart (int iDir)
{
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::ScrollCurrPartToLimit (BOOL bTop)
{
	int iDay = m_Date.GetDay ();
	int iMonth = m_Date.GetMonth ();
	int iYear = m_Date.GetYear ();
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::PushDigit (int iDigit)
{
	int iDay = m_Date.GetDay ();
	int iMonth = m_Date.GetMonth ();
	int iYear = m_Date.GetYear ();
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::ChangeMonth (UINT uiMonthLetter)
{
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::ChangeAmPm (UINT uiAmPm)
{
	int iDay = m_Date.GetDay ();
	int iMonth = m_Date.GetMonth ();
	int iYear = m_Date.GetYear ();
...
//*****************************************************************************************
void CBORROW_DateTimeCtrl::ToggleCheck ()
{
...
//*****************************************************************************************
BOOL CBORROW_DateTimeCtrl::IsDateValid (COleDateTime& date) const
{
...
//*****************************************************************************************
BOOL CBORROW_DateTimeCtrl::IsDatePart (int iPart) const
{
	PART_TYPE type = m_arPartsOrder [iPart];
	return type == DAY || type == MONTH || type == YEAR;
}
//*****************************************************************************************
BOOL CBORROW_DateTimeCtrl::IsTimePart (int iPart) const
{
	PART_TYPE type = m_arPartsOrder [iPart];
	return type == HOUR || type == MIN;
}
//*****************************************************************************************
void CBORROW_DateTimeCtrl::OnHideCalendarPopup ()
{
	//m_bDropButtonIsPressed = FALSE;
...
//****************************************************************************************
void CBORROW_DateTimeCtrl::OnSetFocus(CWnd* pOldWnd) 
{
...
//****************************************************************************************
BOOL CBORROW_DateTimeCtrl::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult) 
{
	if (wParam == iSpinID)
	{
...
//****************************************************************************************
void CBORROW_DateTimeCtrl::OnKillFocus(CWnd* pNewWnd) 
{
	m_bShowSelection = FALSE;
	m_iPrevDigit = -1;
	RedrawWindow ();
...
//****************************************************************************************
void CBORROW_DateTimeCtrl::BuidWidestDate (CDC* pDC)
{
	ASSERT_VALID (pDC);
...
//****************************************************************************************
void CBORROW_DateTimeCtrl::SizeToContent ()
{
	if (GetSafeHwnd () != NULL)
	{
		CRect rectClient;
...
//*************************************************************************************
void CBORROW_DateTimeCtrl::SetAutoResize(BOOL bSet)
{
	m_bAutoResize = bSet;
...
//*************************************************************************************
void CBORROW_DateTimeCtrl::OnDateChanged ()
{
...
//*************************************************************************************
void CBORROW_DateTimeCtrl::SetFirstDayOfWeek(int nDay)	// 0 - 6
{
	m_weekStart = nDay + 1;
}
//*************************************************************************************
UINT CBORROW_DateTimeCtrl::GetState () const
{
	UINT stateFlags = 0;
...
//*************************************************************************************
void CBORROW_DateTimeCtrl::SetState (UINT stateFlags, UINT stateMask)
{
...
//*************************************************************************************
void CBORROW_DateTimeCtrl::PreSubclassWindow() 
{
	if (!m_bIsInitialized)
	{
...
//*************************************************************************************
BOOL CBORROW_DateTimeCtrl::PreCreateWindow(CREATESTRUCT& cs) 
{
...
//*************************************************************************************
BOOL CBORROW_DateTimeCtrl::OnEraseBkgnd(CDC* /*pDC*/)
{
	return TRUE;
}
//*************************************************************************************
void CBORROW_DateTimeCtrl::OnNcCalcSize(BOOL bCalcValidRects, NCCALCSIZE_PARAMS FAR* lpncsp) 
{
...
//*************************************************************************************
void CBORROW_DateTimeCtrl::OnNcPaint() 
{
	if (GetStyle () & WS_BORDER)
	{
...
//*************************************************************************************
BOOL CBORROW_DateTimeCtrl::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message) 
{
	if (m_bIsChecked)
	{
		CPoint ptCursor;
...
//*****************************************************************************
LRESULT CBORROW_DateTimeCtrl::OnSetFont (WPARAM wParam, LPARAM lParam)
{
...
//*****************************************************************************
LRESULT CBORROW_DateTimeCtrl::OnGetFont (WPARAM, LPARAM)
{
	return (LRESULT) m_hFont;
}
//******************************************************************************
//void CBORROW_DateTimeCtrl::OnSize(UINT nType, int cx, int cy) 
//{
...
//}
//*******************************************************************************
void CBORROW_DateTimeCtrl::SetTextColor (COLORREF color, BOOL bRedraw)
{
	m_colorText = color;
...
//*******************************************************************************
void CBORROW_DateTimeCtrl::SetBackgroundColor (COLORREF color, BOOL bRedraw)
{
...
//*******************************************************************************
CString CBORROW_DateTimeCtrl::ToString () const
{
	CString strResult;
...
//*******************************************************************************
void CBORROW_DateTimeCtrl::ClosePopupCalendar (COleDateTime date/* = COleDateTime ()*/)
{
	if (m_pPopup == NULL)
	{
		return;
...
//*******************************************************************************
void CBORROW_DateTimeCtrl::OnDestroy() 
{
	ClosePopupCalendar ();
	CButton::OnDestroy();
}
//*******************************************************************************
void CBORROW_DateTimeCtrl::OnEnable(BOOL bEnable) 
{
