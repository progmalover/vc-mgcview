...
class BORROW_DLLEXPORT CBORROW_DateTimeCtrl : public CButton
{
	friend class CBORROW_Calendar;
	//friend class CBCGPDateTimeXCtrl;
	DECLARE_DYNAMIC(CBORROW_DateTimeCtrl)

// Construction
public:
	CBORROW_DateTimeCtrl();

// Attributes
public:
	void EnableCheckBox(BOOL bEnable = TRUE){ 
		m_bCheckBoxIsAvailable = bEnable;
