...
CAMSEdit::MaskedBehavior::MaskedBehavior(CAMSEdit* pEdit, const CString& strMask /*= _T("")*/) :
	...
	m_strMask(strMask)
{
	m_arraySymbols.Add(Symbol(_T('#'), (Symbol::ValidationFunction) _istdigit));  // default mask symbol
}
...
CString CAMSEdit::MaskedBehavior::_GetValidText() const
{
	...
	for (int iPos = 0, iMaskPos = 0, nLen = strText.GetLength(); iPos < nLen; iPos++, iMaskPos++)
	{
		...
		int iSymbol;
		INT_PTR nSymbolCount;
		for (iSymbol = 0, nSymbolCount = m_arraySymbols.GetSize(); iSymbol < nSymbolCount; iSymbol++)
		{
...
BEGIN_MESSAGE_MAP(CAMSAlphanumericEdit, CAMSEdit)
...
BEGIN_MESSAGE_MAP(CAMSMaskedEdit, CAMSEdit)
...
BEGIN_MESSAGE_MAP(CAMSNumericEdit, CAMSEdit)
...
BEGIN_MESSAGE_MAP(CAMSIntegerEdit, CAMSNumericEdit)
...
BEGIN_MESSAGE_MAP(CAMSCurrencyEdit, CAMSNumericEdit)
...
BEGIN_MESSAGE_MAP(CAMSDateEdit, CAMSEdit)
...
BEGIN_MESSAGE_MAP(CAMSTimeEdit, CAMSEdit)
...
LPARAM CAMSTimeEdit::OnPaste(WPARAM, LPARAM)
{
	return _OnPaste(0, 0);
}
...
BEGIN_MESSAGE_MAP(CAMSDateTimeEdit, CAMSEdit)
...
// Handles the WM_PASTE message to ensure that the text being pasted is a valid date.
LPARAM CAMSDateTimeEdit::OnPaste(WPARAM, LPARAM)
{
	return _OnPaste(0, 0);
}
...
void CAMSMultiMaskedEdit::SetMask(const CString& strMask)
{
	...
	int iPos, nSmallLen;
	for (iPos = 0, nSmallLen = strSmallMask.GetLength(); iPos < nSmallLen; iPos++)
	{
...
BEGIN_MESSAGE_MAP(CAMSMultiMaskedEdit, CAMSEdit)
