// F:\p4\sw\Demo\ContentManagement\Controller\Alarm\AlarmSystem.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CAlarmSystemPage, CBaseControllerPage)


CAlarmSystemPage::CAlarmSystemPage()
{
}
...
BEGIN_MESSAGE_MAP(CAlarmSystemPage, CBaseControllerPage)
    ON_WM_CREATE()
    ON_WM_DESTROY()
...
/// CAlarmSystemPage message handle
int CAlarmSystemPage::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
    ...
    // Create and Install the Alarm search pane
    right   = rect.Width() / 4;
    bottom  = rect.bottom;

    if (!m_wndAlarmSearch.Create(Translate(_T("Alarm Search")), this, CRect(left, top, right, bottom),
        TRUE, ID_VIEW_LOGSEARCH, WS_CHILD | WS_VISIBLE | CBRS_LEFT | CBRS_HIDE_INPLACE | WS_CAPTION,
        AFX_CBRS_REGULAR_TABS, AFX_CBRS_RESIZE))
    {
        TRACE0("Failed to create Alarm search pane\n");
        return -1;      // fail to create
    }
    m_wndAlarmSearch.EnableDocking(CBRS_ALIGN_ANY);
    DockPane(&m_wndAlarmSearch);

    // Create and Install the Alarm list view pane
    left    = right;
    right   = rect.right;

    if (!m_wndAlarmList.Create(Translate(_T("Alarm View")), this, CRect(left, top, right, bottom),
        TRUE, ID_VIEW_LOGLIST, WS_CHILD | WS_VISIBLE |  CBRS_LEFT | CBRS_HIDE_INPLACE | WS_CAPTION,
        AFX_CBRS_REGULAR_TABS, AFX_CBRS_RESIZE))
    {
        TRACE0("Failed to create Alarm list view pane\n");
        return -1;      // fail to create
    }

    SetMainWnd(&m_wndAlarmList);
...
void CAlarmSystemPage::OnDestroy()
{
...
void CAlarmSystemPage::Close()
{
    m_wndAlarmSearch.DestroyWindow();
    m_wndAlarmList.DestroyWindow();
}

BOOL CAlarmSystemPage::CreateRibbonButtons()
{
    if (!m_pCategory)
    {
        return FALSE;
...
void CAlarmSystemPage::OnRibbonBtnRefresh()
{
    m_wndAlarmSearch.m_AlarmFilter.OnBnClickedButtonSearch();
}

void CAlarmSystemPage::OnRibbonBtnPrint()
{
    return;
    CDC dc;
...
void CAlarmSystemPage::OnUpdateRefresh(CCmdUI* pCmdUI)
{
    if (!IsEnable())
    {
        pCmdUI->Enable(FALSE);
...
void CAlarmSystemPage::OnUpdatePrint(CCmdUI* pCmdUI)
{
    if (!IsEnable())
    {
        pCmdUI->Enable(FALSE);
