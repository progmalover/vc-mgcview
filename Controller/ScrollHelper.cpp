...
void CScrollHelper::SetDialogUnitSize( int dluX, int dluY )
{
    CRect rect(0, 0, dluX, dluY);

    MapDialogRect(m_attachWnd->GetSafeHwnd(), rect);

    LONG xy = GetDialogBaseUnits();
    int tx = LOWORD(xy);
    int ty = HIWORD(xy);

    int pixelX = MulDiv(tx, dluX, 4);
    int pixelY = MulDiv(ty, dluY, 8);

    SetDisplaySize(pixelX, pixelY);
}
